...
class DataSelectionStrategy(object):
    ...
    def compute_gradients(self, valid=False, perBatch=False, perClass=False):
        ...
        ## vqa
        for batch_idx,(image, question, answer, weights, n) in tqdm(enumerate(trainloader)):
            image, weights = image.to(self.device,non_blocking=True), weights.to(self.device,non_blocking=True) 
        ## caption
        #for batch_idx, (image, caption) in tqdm(enumerate(trainloader)):
        #    image = image.to(self.device) 
            #inputs, targets = inputs.to(self.device), targets.to(self.device, non_blocking=True)
            if batch_idx == 0:
                loss,out = self.model.forward_coreset(image, question, answer, train=True, n=n, weights=weights) 
                ##caption
                #loss, out = self.model.forward_coreset(image, caption) 
                #out, l1 = self.model(inputs, last=True, freeze=True)
                #loss = self.loss(out, targets).sum()
                l0_grads = torch.autograd.grad(loss, out.logits)[0]
                ...
                batch_l0_grads = torch.autograd.grad(loss, out.logits)[0]
                if self.linear_layer:
                ...
                l0_grads = batch_l0_grads #torch.cat((l0_grads, batch_l0_grads), dim=0)
                if self.linear_layer:
        ...
        if valid:
            ...
            for batch_idx,(image, question, answer, weights, n) in tqdm(enumerate(valloader)):
                image, weights = image.to(self.device,non_blocking=True), weights.to(self.device,non_blocking=True) 
            #for batch_idx, (image, caption) in tqdm(enumerate(valloader)):
            #   image = image.to(self.device) 
                #inputs, targets = inputs.to(self.device), targets.to(self.device, non_blocking=True)
                if batch_idx == 0:
                    loss,out = self.model.forward_coreset(image, question, answer, train=True, n=n, weights=weights) 

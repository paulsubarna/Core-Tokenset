...
def select_coreset(model,task, transform, args):
    if task ==1:
        datasets= vqa_dataset( transform, ann_root= config['ann_root'] , vqa_root= config['vqa_root'], mem_root=config['mem_root'], vg_root=config['vg_root'], doc_root= config['clevr_root'], train_files=['vqa_train' ], split="train", task=task)
        val_datasets= vqa_dataset( transform, ann_root= config['ann_root'] , vqa_root= config['vqa_root'], mem_root=config['mem_root'], vg_root=config['vg_root'], doc_root= config['clevr_root'], train_files=['vqa_train_test'  ], split="test", task=0)
        ...
        datasets= vqa_dataset( transform,  ann_root= config['ann_root'] , vqa_root= config['vqa_root'], mem_root=config['mem_root'], vg_root=config['vg_root'], doc_root= config['clevr_root'], train_files=[ 'CLEVR_train' ], split="train", task=task)
        
        val_datasets= vqa_dataset( transform,  ann_root= config['ann_root'] , vqa_root= config['vqa_root'], mem_root=config['mem_root'], vg_root=config['vg_root'], doc_root= config['clevr_root'], train_files=[ 'CLEVR_val' ], split="test", task=task)
...
def update_annotation(model, ann_root, task, transform, args):
    ...
    for i in tqdm(range(len(annotation))):
        if i in cr_idx:
            ...
            attn= model.forward_atman(image) 
...
def main(args, config):
    ...
    for t in range(args.num_task):
        if t ==0 or t==1:
            ...
            if t ==0:
                datasets= vqa_dataset( transform,  ann_root= config['ann_root'] , vqa_root= config['vqa_root'], mem_root=config['mem_root'], vg_root=config['vg_root'], doc_root= config['clevr_root'], train_files=['vqa_train'], split="train", task=t)
            elif t ==1:
                datasets= vqa_dataset( transform,  ann_root= config['ann_root'] , vqa_root= config['vqa_root'], mem_root=config['mem_root'], vg_root=config['vg_root'], doc_root= config['clevr_root'], train_files=['vqa_train_cr10', 'CLEVR_train' ], split="train", task=t)
            else:
                datasets= vqa_dataset( transform,  ann_root= config['ann_root'] , vqa_root= config['vqa_root'], mem_root=config['mem_root'], vg_root=config['vg_root'], doc_root= config['clevr_root'], train_files=['vqa_train_cr10', 'CLEVR_train_cr10', 'vg_qa' ], split="train", task=t)

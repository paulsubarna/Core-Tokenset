...
def update_annotation(model, ann_root, file, val_file, root, tok_root, transform, args):
    ...
    for i in tqdm(range(len(annotation))):
        if i in cr_idx:
            ...
            attn= model.forward_chefer(image, caption)
    ...
    with open(os.path.join(ann_root,f'{file}_hr10.json'), "w") as jsonFile:
        json.dump(temp_annot, jsonFile)

    annota = json.load(open(os.path.join(ann_root,f'{file}_hr10.json'),'r'))
    print(len(annota))
...
def main(args, config):
    ...
    #train_dataset, val_dataset, test_dataset = create_dataset('caption_coco', config)   #config['pretrained']
    ...
    ## load previous model 
    model.load_state_dict(torch.load(config['trained_model_path'], map_location=torch.device('cuda:0')))
    ...
    files= [ 'coco_backpack_train', 'coco_book_train', 'coco_truck_train', 'coco_bowl_train', 'coco_bottle_train', 'coco_cup_train', 'coco_chair_train', 'coco_car_train'] #
    ...
    cor_files= ['coco_backpack_train_cr10', 'coco_book_train_hr10', 'coco_truck_train_hr10' , 'coco_bowl_train_hr10', 'coco_bottle_train_hr10']

    i_root = ['/app/src/BLIP/dataset/coco_backpack','/app/src/BLIP/dataset/coco_book', '/app/src/BLIP/dataset/coco_truck' , '/app/src/BLIP/dataset/coco_bowl',  '/app/src/BLIP/dataset/coco_bottle', '/app/src/BLIP/dataset/coco_cup', '/app/src/BLIP/dataset/coco_chair',  '/app/src/BLIP/dataset/coco_car'  ] 
    tok_root = ['/app/src/BLIP/dataset/coco_token_backpack','/app/src/BLIP/dataset/coco_token_book', '/app/src/BLIP/dataset/coco_token_truck' , '/app/src/BLIP/dataset/coco_token_bowl',  '/app/src/BLIP/dataset/coco_token_bottle', '/app/src/BLIP/dataset/coco_token_cup', '/app/src/BLIP/dataset/coco_token_chair',  '/app/src/BLIP/dataset/coco_token_car'  ] 
    ...
    for t in range(args.num_tasks):
        if t>0:
            ...
            for epoch in range(0, config['max_epoch']):
                ...
                if (epoch+1)%50 == 0:
                    torch.save(model.module.state_dict(),f'set_path_{t}_{epoch}.pt') ### Define the output location
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='Core-Token/Multimodal_tasks/configs/caption_coco.yaml')

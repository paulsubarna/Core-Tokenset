...
def select_coreset(model,task, transform, args):
    ...
    cfg= load_config_data('/app/src/BLIP/coreset/configs/T-IMGNET/craig/craig_img_vit16.py')

    logger = logging.getLogger(__name__)

    dataloader = CRAIGDataLoader(model, train_loader, val_loader, cfg, logger,
    ...
    np_idx = np.array(idx)
    with open('/app/src/BLIP/index_cg10.npy', 'wb') as f:
        np.save(f, np_idx)
...
def update_annotation(model, ann_root, task, transform, args):
    ...
    if task ==1:
        ...
        with open(f'/app/src/BLIP/dataset/vqa/vqa_train_cg10.json', "w") as jsonFile:
            json.dump(temp_annot, jsonFile)
        ann_root = '/app/src/BLIP/dataset/vqa'
        annota = json.load(open(os.path.join(ann_root,'vqa_train_cg10.json'),'r'))  
        print(len(annota))
        ...
        for ids in tqdm(range(len(annotation))):
            if ids in cr_idx:
                temp_annot.append(annotation[ids])
        with open(f'/app/src/BLIP/dataset/vqa/CLEVR_train_cg10.json', "w") as jsonFile:
            json.dump(annotation, jsonFile)
        ann_root = '/app/src/BLIP/dataset/vqa'
        annota = json.load(open(os.path.join(ann_root,'CLEVR_train_cg10.json'),'r'))  
        print(len(annota))
    ...
    for t in range(args.num_task):
        if t ==0:
            ...
            if t ==0:
                ...
                datasets= vqa_dataset( transform, ann_root= '/app/src/BLIP/dataset/vqa' , vqa_root= '/app/src/BLIP/dataset/coco_train2014',  vg_root='/app/src/BLIP/dataset/images/', mem_root='/app/src/BLIP/dataset/token_image10/', doc_root= '/app/src/BLIP/dataset/CLEVR_v1.0/images/train', train_files=['vqa_train_cg10', 'CLEVR_train' ], split="train", task=t)
            else:
                datasets= vqa_dataset( transform, ann_root= '/app/src/BLIP/dataset/vqa' , vqa_root= '/app/src/BLIP/dataset/coco_train2014',vg_root='/app/src/BLIP/dataset/images/',  mem_root='/app/src/BLIP/dataset/token_image10/',  doc_root= '/app/src/BLIP/dataset/CLEVR_v1.0/images/train', train_files=['vqa_train_cg10', 'CLEVR_train_cg10', 'vg_qa' ], split="train", task=t)
            ...
            for epoch in range(0, config['max_epoch']):
                ...
                if (epoch+1)%2 == 0:
                    torch.save(model.module.state_dict(),f'/app/src/BLIP/output/VQA/BLIP_CSCRG10_{t}_{epoch}.pt') 
                    torch.save(optimizer.state_dict(),f'/app/src/BLIP/output/VQA/opt_CSCRG10_{t}_{epoch}.pt')
                dist.barrier()         

...
def update_annotation(model, ann_root, task, transform, args):
    ...
    if task == 1:
        annotation = json.load(open(os.path.join(ann_root,f'vqa_train_90.json'),'r'))
        img_root = '/app/src/BLIP/dataset/coco_train2014'
    elif task == 2:
        annotation = json.load(open(os.path.join(ann_root,f'CLEVR_train_90.json'),'r'))
    ...
    for i in tqdm(range(len(annotation))):
        if i >0:
            ...
            save_image(img, f'/app/src/BLIP/dataset/token_image_90/coco_{index}.png')
    ...
    if task ==1:
        with open(f'/app/src/BLIP/dataset/vqa/vqa_train_90.json', "w") as jsonFile:
            json.dump(annotation, jsonFile)
    elif task ==2:
        with open(f'/app/src/BLIP/dataset/vqa/CLEVR_train_90.json', "w") as jsonFile:
            json.dump(annotation, jsonFile)
...
def main(args, config):
    ...
    model = blip_vqa(pretrained=None, image_size=config['image_size'], 
                       vit=config['vit'], vit_grad_ckpt=config['vit_grad_ckpt'], vit_ckpt_layer=config['vit_ckpt_layer'])
    model.load_state_dict(torch.load('/app/src/BLIP/output/VQA/BLIPAC_0_19.pt', map_location= torch.device('cuda:0')))
    ...
    for t in range(args.num_task):
        if t==0 : #or t==1:
            ...
            for epoch in range(0, config['max_epoch']):
                ...
                #if (epoch+1)%2 == 0:
                torch.save(model.module.state_dict(),f'/app/src/BLIP/output/VQA/BLIP_HCA_{1-args.ratio}_{t}_{epoch}.pt') 
                torch.save(optimizer.state_dict(),f'/app/src/BLIP/output/VQA/opt_HCA_{1-args.ratio}_{t}_{epoch}.pt')
                dist.barrier()         
